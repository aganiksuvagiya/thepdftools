...
import {
  PNG_TO_JPG_VARIANTS,
  type PngToJpgVariant,
} from "@/lib/png-to-jpg-variants";

const PngToJpgClient = dynamic(
  () => import("../app/png-to-jpg/PngToJpgClient"),
  {
...
export function buildPngToJpgVariantMetadata(
  variant: PngToJpgVariant
): Metadata {
...
export default function PngToJpgVariantPage({
  variant,
}: {
  variant: PngToJpgVariant;
}) {
  const pageUrl = `${SITE_URL}/${variant.slug}`;
  const siblingVariants = PNG_TO_JPG_VARIANTS.filter(
  ...
  const jsonLd = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        keywords: [variant.primaryKeyword, "png to jpg online free no upload"],
        description: variant.description,
      },
      {
        "@type": "FAQPage",
        ...
        <div className="mt-14 space-y-8">
          ...
          <section className="rounded-[1.75rem] border border-brand-100 bg-brand-50 p-8 shadow-sm">
            ...
            <p className="mt-4 text-[15px] leading-8 text-slate-700">
              After converting PNG to JPG, you may want to compress the output
              further, resize it for a specific platform, convert it to WebP for
              modern web delivery, or switch it back to PNG when lossless storage
              matters again.
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              <Link
                href="/png-to-jpg"
                className="rounded-full border border-brand-200 bg-white px-4 py-2 text-sm font-medium text-brand-700 shadow-sm transition-colors hover:border-brand-300 hover:text-brand-800"
              >
                Main PNG to JPG Tool
              </Link>
              <Link
                href="/jpg-to-png"
